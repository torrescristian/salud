...
export interface MedicationRepository {
  ...
  getMedicationSuggestions(): Array<{
    name: string;
    usageCount: number;
    lastUsed: Date;
  }>;
...
class LocalStorageMedicationRepository implements MedicationRepository {
  ...
  getMedications(): UserMedication[] {
    const medications = this.getItem<UserMedication[]>(
    ...
    return medications.map((med) => ({
      ...med,
      lastUsed: migrateUTCToLocal(med.lastUsed.toString()),
    }));
  }
  ...
  getMedicationSuggestions(): Array<{
    ...
    const suggestions = this.getItem<
      Array<{ name: string; usageCount: number; lastUsed: string }>
    >(STORAGE_KEYS.SUGGESTIONS);
    if (!suggestions) return [];

    return suggestions.map((suggestion) => ({
      ...suggestion,
      lastUsed: migrateUTCToLocal(suggestion.lastUsed.toString()),
    }));
  }
